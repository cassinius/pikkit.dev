...
const PLAYBOOKS_DIR = process.env.PLAYBOOKS_DIR ?? "./db/seeds/playbooks";
...
for (const file of files) {
  ...
  console.log(
    `Parsed ${parsedAtoms.length.toString().padStart(2, " ")} atoms from ${file}`,
  );
...
console.log(
  `Seeded ${atomCount} playbook atoms. atoms table now has ${count} rows.`,
);
